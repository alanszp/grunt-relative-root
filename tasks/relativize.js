...
module.exports = function(grunt) {
  ...
  grunt.registerMultiTask('relativize', pkg.description, function() {
    ...
    this.files.forEach(function(file) {
      var src = file.src[0],
          ...
          relativeRoot, filter, contents;

          if (file.forceRelative != null) {
            relativeRoot = file.forceRelative;
            var endsWithSlash = relativeRoot.indexOf('/', relativeRoot.length - 1) !== -1;
            if (!endsWithSlash) {
              relativeRoot += '/';
            }
          }
